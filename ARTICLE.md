...
> patterns that work are exploitable, and the benchmark's own gold answers
> are more dangerous than the models it grades, and more dangerous than the
> regular expressions people actually ship.
...
1. Nobody had run the safety screen on the benchmark's **own human
   reference answers**, or against the regular expressions in shipped code.
...
## So we checked three more populations
...
[NL-RX][nlrx], and, most importantly, **537,806 regular expressions
extracted from shipped packages** across npm, PyPI, Maven, CPAN, crates.io,
godoc, packagist and RubyGems by [Davis et al.][linguafranca], the artifact
of the paper that measured ReDoS at ecosystem scale.

| where the patterns come from | n | vulnerable |
| --- | ---: | ---: |
| NL-RX, generated by a grammar *(control)* | 5,840 | 35.2% |
| KB13 gold answers, human | 532 | 16.7% |
| Re(gEx\|DoS)Eval gold answers, human | 755 | 13.1% |
| **our eleven models, pooled** | — | **9.0%** |
| **production code, human** | 5,000 | **4.9%** |

Real shipped code is safer than every model we tested. So the endemic
reading is wrong, and we would have published it.

There is an obvious objection, and it is a good one. This corpus is full of
validators, email and ISBN and hostname, which is exactly the shape that
backtracks. Production code is mostly small utility patterns that never had
the chance. So we restricted both sides to anchored `^...$` patterns, the
closest matched populations we can build:

| anchored patterns only | n | vulnerable |
| --- | ---: | ---: |
| Re(gEx\|DoS)Eval gold answers | 538 | 13.4% ±2.9 |
| Production code | 4,000 | 8.9% ±0.9 |

Most of the gap was task mix. What is left is smaller and holds up:

> **Models write regular expressions about as dangerous as the ones people
> actually ship. Both are safer than the answer keys they are graded
> against.**

Models at 9.0% and production code at 8.9% are the same number. The answer
key is the outlier, and it is the outlier in the same direction as
everything else we found about it. We already knew it was semantically wrong
in about a third of the disagreements we read. It also turns out to be the
most ReDoS-prone population of human-written regular expressions we could
find.

That is not a mystery once stated. A reference answer is written once to
satisfy a description, and then nothing ever happens to it. Nobody puts
traffic through it. Nobody reviews it. Nobody files a CVE against it. The
patterns in shipped packages have been through some of that, and a few of
them have been through it specifically for ReDoS, because Davis and
colleagues went and told people. Answer keys get none of that maintenance,
which is precisely why treating them as ground truth is a mistake.

The practical advice does not change, only its justification. If you want
safe regular expressions you have to screen for them, because nobody in this
picture is doing it for you.
