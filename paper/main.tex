...
it with a sharper ordering. Controlling for task mix, patterns \emph{written
to be read} are ReDoS-prone (reusable library entries 20.1\%, Stack Overflow
answers 17.3\%, this corpus's reference answers 13.4\%), while patterns that
are \emph{executed} in shipped packages sit at 8.9\%, statistically
indistinguishable from the models at 9.0\%. \emph{Vulnerability tracks
whether a pattern was ever run, not whether a human or a model wrote it.}
...
number does not: the same eleven models leave \textbf{16.5\%} of their
correct patterns ReDoS-vulnerable there against 7.4\% here, on tasks twenty
...
\begin{enumerate}
  ...
  configuration. The finding replicates in kind and not in size: 16.5\%
  against 7.4\%, on easier tasks, with the model ordering not preserved. This
...
\caption{The identical ReDoS screen applied to six populations. Intervals are
95\% binomial. Model figures are the pooled per-sample rate from
\cref{tab:vuln}. The anchored block is the comparison to read: the
...
20.1\%, forum answers at 17.3\%, benchmark reference answers at 13.4\% ---
is ReDoS-prone. The one population that has been \emph{executed} under real
traffic sits at 8.9\%, and the models sit with it at 9.0\%.
\end{quote}
...
source population (13.4\%) suggests its authors curated, which is to their
...
Every figure above comes from one corpus, so the 7.4\% is a property of
...
by all eleven models. Percentages. The last column is the share of
functionally correct patterns that are ReDoS-vulnerable, the same quantity
this paper reports as 7.4\% on Re(gEx$|$DoS)Eval.}
\label{tab:srcommon}
\end{table}
...
finding. But the rate is \textbf{16.5\%} here against 7.4\% on
Re(gEx$|$DoS)Eval, and the obvious deflationary reading, that harder problems
produce worse patterns, is ruled out by the direction of the difficulty gap:
StructuredRegex is \emph{easier}, by roughly twenty points of $\pass{}@1$.
Its targets are built from a grammar over repetition, optionality and
concatenation, and models answering those descriptions write more
backtracking-prone patterns than models answering the validator-heavy prompts
of Re(gEx$|$DoS)Eval.
...
reference-independent gap is 7.4\% on one corpus and 16.5\% on another under
...
flattered the affected model rather than penalised it.
\end{enumerate}
...
indistinguishable from the regular expressions in shipped packages. Across
...
the answer key rather than in generation or in human practice. And the entire eleven-model field lies within eight percentage
points across a $98\times$ range in inference cost.

A third check went the same way. The 7.4\% that survived the first two
corrections is 16.5\% on a second corpus under the same screen and the same
...
component over a hard core the filter rejects every time. Final coverage was
513 of 622.
