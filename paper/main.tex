...
finds roughly half of correct backends exploitable, SecureAgentBench 15.2\%
correct-and-secure for its best agent. The regex domain does \emph{not}
...
in approximately 15\% of cases; the remainder divided between demonstrably
incorrect reference patterns (including a reference character class containing
...
headline gap, the composite was measuring its own weakest component. We further show that independent binomial intervals---the
...
\begin{enumerate}
  ...
  (\cref{sec:jointprior}); our contribution is its transfer to
  ...
  exploits rather than comparing against a gold artifact---so this failure mode
  does not arise there. It arises here because the NL-to-regex literature is
...
Our results are consistent with all four and, we argue, strengthened by that
consistency; \cref{sec:crossbench} makes the comparison quantitative. The
present work differs in domain rather than in framing: the artifact is a
regular expression rather than a program, the vulnerability class is ReDoS
rather than a CWE taxonomy, correctness is partly decidable rather than
established solely by tests, and---because the NL-to-regex literature supplies
a per-task human reference---a same-instrument human baseline is available.
That last difference is what makes \cref{sec:humanbaseline} possible and is
unavailable in reference-free joint benchmarks.
...
natural thing to do; the point of the following section is that doing so
without decomposing it is a mistake we made and had to correct.
...
The safety term costs \textbf{2.9 percentage points} on average. The
equivalence term costs \textbf{18.9}. The equivalence conjunct therefore
supplies \textbf{87\%} of the gap that motivated the composite.

This matters because \cref{sec:goldstandard} establishes independently that
the equivalence term is approximately 85\% reference-set error and task
underspecification rather than model error. The composite's headline result
was, in the main, a measurement of its own least reliable component. We
report this because we did not anticipate it: the decomposition was run late,
after the audit, and it changed the paper's claim.
...
simultaneously screened as unsafe. Expressed as a rate over correct
generations rather than as a count, this is the 7.4\% above---approximately
one in fourteen, not one in two.
...
methods differ substantially; the comparison is of magnitude and of the ratio
between functional and joint success, not of models or difficulty.}
\label{tab:crossbench}
\end{table}

\textbf{We do not reproduce the pattern.} \citet{vero2025baxbench} report
that roughly half of functionally correct backends are exploitable;
\citet{chen2025secureagentbench} report 15.2\% correct-and-secure for their
strongest agent against substantially higher functional correctness;
\citet{patir2025dualgauge} report secure-pass@1 below 12\% while functional
pass@1 exceeds 50\%. In each case the security criterion removes a large
majority of functionally correct output.

In our setting it removes 7.4\%.

We consider the divergence more informative than agreement would have been,
and offer three candidate explanations, which we cannot separate with this
data:
...
The generalisable claim is therefore weaker than ``correct code is often
insecure'' and, we think, more useful: \emph{the size of the
correctness-to-security penalty is domain-specific and must be measured
per domain, not assumed}. Reporting a joint metric without decomposing it
would have concealed this in our case (\cref{sec:decomp}), since our composite
showed a gap of the expected magnitude for entirely the wrong reason.
...
We set out to measure a correctness-to-shippability gap in generated
regular expressions, constructed a composite metric that appeared to show a
large one, and found on decomposition that 87\% of it came from the single
term we had independently shown to be unreliable. The defensible version of
the result is considerably smaller: 7.4\% of functionally correct patterns
carry a ReDoS vulnerability, against roughly 50\% for functionally correct
backends in comparable work. The correctness-to-security penalty is
domain-specific; ours is small.
...
The methodological contribution is the one we did not plan. A metric that
compares against a human answer key inherits that key's error rate; a
composite containing such a term can present a large, plausible, and
essentially spurious result. We established this only by auditing our own
output after collecting it, and it changed what we were willing to publish.
We recommend the practice generally, and note that the cost of the audit was
a few hours against a run of \$10.95.
